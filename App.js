...
const App = () => {
  ...
  const [silverCoins, setSilverCoins] = useState(10000);
  ...
  const startRotation = () => {
    ...
    // if (!selectedCoin || !selectedWheelItem) {
    //   Alert.alert("Action Denied", "Please select a coin and an icon before spinning!");
    //   return;
    // }
    ...
    const spinDuration = 5000; // Total spin time
    ...
    Animated.timing(rotation, {
      ...
      const selectedSegmentIndex =
        Math.floor(numSegments - (normalizedRotation / 360) * numSegments) % numSegments;
      ...
      // Check if the result matches the user's selection
      if(selectedCoin || selectedWheelItem){
      if (selectedWheelItem.id === selectedItem.id) {
        console.log('hello',selectedItem)
        // Match: Add coins
        setSilverCoins(prev => prev + selectedCoin * parseInt(selectedItem.times));
        Alert.alert("Congratulations!", `You won! ${selectedCoin * parseInt(selectedItem.times)} silver coins.`);
      } else {
        // No match: Subtract coins
        setSilverCoins(prev => prev - selectedCoin * parseInt(selectedWheelItem.times));
        Alert.alert("Sorry!", `You lost! ${selectedCoin * parseInt(selectedWheelItem.times)} silver coins.`);
      }
  ...
  const handleCoinPress = value => {
    if (isSpinning) {
      Alert.alert("Action Denied", "You can't select a coin during spinning!");
      return;
    }
  ...
  const handleIconPress = item => {
    if (isSpinning) {
      Alert.alert("Action Denied", "You can't select an icon during spinning!");
      return;
    }
  ...
  return (
    <ImageBackground source={require('./src/images/image.jpeg')} style={styles.background}>
      ...
      <View style={styles.wheelContainer}>
        <Animated.View style={[styles.wheel, { transform: [{ rotate: interpolatedRotation }] }]}>
          {wheelItems.map((item, index) => {
            ...
            return (
              <TouchableOpacity key={item.id} onPress={() => handleIconPress(item)} style={[styles.wheelItem, { top: 150 + y - 30, left: 150 + x - 30 }, isSelected && { transform: [{ scale: selectedScale }] }]}>
                <View style={styles.imageWrapper}>
                  <Image source={item.icon} style={styles.wheelIcon} />
                </View>
                <Text style={[styles.wheelText, { fontWeight: 'bold' }]}>{item.times}</Text>
              </TouchableOpacity>
            );
          })}
        </Animated.View>
      ...
      <View style={styles.betContainer}>
        <Text style={styles.betText}>Choose the amount and then choose food...</Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {coinItems.map(item => (
            <TouchableOpacity key={item.id} onPress={() => handleCoinPress(item.value)}>
              <Image source={item.icon} style={styles.betImage} />
            </TouchableOpacity>
          ))}
        </ScrollView>
        {selectedCoin && <Text style={styles.selectedText}>You selected: {selectedCoin}</Text>}
      </View>
      ...
      <View style={styles.footerContainer}>
        ...
        <TouchableOpacity style={styles.footerButton}>
          <Text>Silver Coins: {silverCoins}</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.Rankcontainer}>
        ...
        <View>
          <Image source={require('./src/images/spin.webp')} style={styles.resultImage} />
        </View>
        <View>
